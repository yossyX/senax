...
use crate::models::@{ group_name|snake|ident }@::@{ mod_name|ident }@::*;
#[allow(unused_imports)]
use anyhow::Context as _;
// #[allow(unused_imports)]
// use domain::repository::@{ db|snake|ident }@::@{ group_name|snake|ident }@::@{ mod_name|ident }@::{self, *};
// use domain::repository::@{ db|snake|ident }@::@{ group_name|snake|ident }@::@{ mod_name|ident }@::*;
use base_domain as domain;
use domain::models::@{ db|snake|ident }@::@{ group_name|snake|ident }@::@{ mod_name|ident }@::*;
#[allow(unused_imports)]
...
@{- parent.relations_many(true)|fmt_rel_join("
    fn {rel_name}(&mut self) -> anyhow::Result<Box<dyn domain::models::UpdateIterator<dyn _model_::{class_mod_path}::{class}Updater> + '_>> {
        ...
        impl<T: crate::misc::Updater + _model_::{class_mod_path}::{class}Updater> domain::models::UpdateIterator<dyn _model_::{class_mod_path}::{class}Updater> for V<'_, T> {
            fn iter_mut(&mut self) -> Box<dyn Iterator<Item = &mut (dyn _model_::{class_mod_path}::{class}Updater + 'static)> + '_> {
                Box::new(self.0.iter_mut().map(|v| v as &mut dyn _model_::{class_mod_path}::{class}Updater))
            }
        }
    ...
    fn take_{raw_rel_name}(&mut self) -> Option<Vec<Box<dyn _model_::{class_mod_path}::{class}Updater>>> {
        __Updater__::mut_{raw_rel_name}(self).take().map(|v| v.into_iter().map(|v| Box::new(v) as Box<dyn _model_::{class_mod_path}::{class}Updater>).collect())
    }
