...
export const @{ selector|pascal }@@{ pascal_name }@Query = graphql(`query @{ selector }@_@{ db }@_@{ group }@_@{ mod_name }@($filter: @{ db|pascal }@@{ group|pascal }@@{ pascal_name }@Query@{ selector|pascal }@Filter@% if selector_def.filter_is_required() %@!@% endif %@, $order: @{ db|pascal }@@{ group|pascal }@@{ pascal_name }@Query@{ selector|pascal }@Order, $limit: Int, $offset: Int){@{ db_case }@{@{ group_case }@{@{ model_case }@{@{ selector|gql_camel }@(filter: $filter, order: $order, first: $limit, offset: $offset){nodes{...@{ db|pascal }@@{ group|pascal }@@{ pascal_name }@}}}}}}`);
export const @{ selector|pascal }@WithCursor@{ pascal_name }@Query = graphql(`query @{ selector }@_with_cursor_@{ db }@_@{ group }@_@{ mod_name }@($filter: @{ db|pascal }@@{ group|pascal }@@{ pascal_name }@Query@{ selector|pascal }@Filter@% if selector_def.filter_is_required() %@!@% endif %@, $order: @{ db|pascal }@@{ group|pascal }@@{ pascal_name }@Query@{ selector|pascal }@Order, $after: String, $before: String, $first: Int, $last: Int, $offset: Int){@{ db_case }@{@{ group_case }@{@{ model_case }@{@{ selector|gql_camel }@(filter: $filter, order: $order, after: $after, before: $before, first: $first, last: $last, offset: $offset){pageInfo{hasPreviousPage,hasNextPage,startCursor,endCursor},nodes{...@{ db|pascal }@@{ group|pascal }@@{ pascal_name }@}}}}}}`);
export const Count@{ selector|pascal }@@{ pascal_name }@Query = graphql(`query count_@{ selector }@_@{ db }@_@{ group }@_@{ mod_name }@($filter: @{ db|pascal }@@{ group|pascal }@@{ pascal_name }@Query@{ selector|pascal }@Filter@% if selector_def.filter_is_required() %@!@% endif %@){@{ db_case }@{@{ group_case }@{@{ model_case }@{count@{ selector|gql_pascal }@(filter: $filter)}}}}`);
@%- endfor %@
@%- endfor %@
