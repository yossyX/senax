...
use crate::misc::{BindArrayTr, BindTr, BindValue, ColRelTr, ColTr, FilterTr, IntoJson as _, OrderTr};
use crate::repositories::*;
use db::misc::{Count, Updater, Size, TrashMode};
use db::models::{ModelTr as _, USE_FAST_CACHE};
use db::{accessor::*, CacheMsg, BULK_INSERT_MAX_SIZE, IN_CONDITION_LIMIT};
pub use db::models::@{ group_name|to_var_name }@::_base::_@{ mod_name }@::*;
            ...
            match self {
                ...
                CacheOp::Insert { replace, shard_id, data
                    ...
                    if USE_UPDATE_NOTICE && DbConn::_has_update_notice() {
                        ...
                        DbConn::_push_update_notice(db::models::TableName::@{ table_name }@, op, &id).await;
                    }
    ...
    @{- def.relations_many_cache(false)|fmt_rel_join("
                ...
                CacheOp::BulkInsert { replace, overwrite, ignore, shard_id, list } => {
                    ...
                    if overwrite {
                        ...
                        if USE_UPDATE_NOTICE && DbConn::_has_update_notice() {
                            DbConn::_push_update_notice(db::models::TableName::@{ table_name }@, db::models::NotifyOp::invalidate_all, &serde_json::Value::Null).await;
                        }
                    }
                    @%- endif %@
                }
                CacheOp::Update { id, shard_id, update, op
                    ...
                    clear_all_rows_cache(shard_id, sync, false).await;
                    if USE_UPDATE_NOTICE && DbConn::_has_update_notice() {
                        DbConn::_push_update_notice(db::models::TableName::@{ table_name }@, db::models::NotifyOp::update, &id).await;
                    }
                    if USE_CACHE {
                ...
                CacheOp::BulkUpsert { shard_id, data_list, update, op } => {
                    ...
                    if USE_UPDATE_NOTICE && DbConn::_has_update_notice() {
                        for data in &data_list {
                            ...
                            DbConn::_push_update_notice(db::models::TableName::@{ table_name }@, db::models::NotifyOp::upsert, &id).await;
                        }
                    }
                    if USE_CACHE {
                        for data in &data_list {
                ...
                CacheOp::Delete { id, shard_id } => {
                    ...
                    clear_all_rows_cache(shard_id, sync, false).await;
                    if USE_UPDATE_NOTICE && DbConn::_has_update_notice() {
                        DbConn::_push_update_notice(db::models::TableName::@{ table_name }@, db::models::NotifyOp::delete, &id).await;
                    }
                    if USE_CACHE {
                        let id = PrimaryHasher(id.clone(), shard_id);
                        @%- if def.versioned %@
                ...
                CacheOp::DeleteAll { shard_id } => {
                    ...
                    _clear_cache(shard_id, sync, false).await;
                    if USE_UPDATE_NOTICE && DbConn::_has_update_notice() {
                        DbConn::_push_update_notice(db::models::TableName::@{ table_name }@, db::models::NotifyOp::delete_all, &serde_json::Value::Null).await;
                    }
                }
                ...
                CacheOp::Invalidate { id, shard_id  } => {
                    ...
                    clear_all_rows_cache(shard_id, sync, false).await;
                    if USE_UPDATE_NOTICE && DbConn::_has_update_notice() {
                        DbConn::_push_update_notice(db::models::TableName::@{ table_name }@, db::models::NotifyOp::invalidate, &id).await;
                    }
                    if USE_CACHE {
                        let id = PrimaryHasher(id.clone(), shard_id);
                        @%- if def.versioned %@
...
pub(crate) async fn _clear_cache(shard_id: ShardId, sync: u64, clear_test: bool) {
    clear_all_rows_cache(shard_id, sync, clear_test).await;
    if USE_CACHE {
...
@%- else %@
pub(crate) use domain::repository::@{ db|snake|to_var_name }@::@{ group_name|to_var_name }@::_base::_@{ mod_name }@::Col_;
@%- endif %@
impl ColTr for Col_ {
...
@%- else %@
pub(crate) use domain::repository::@{ db|snake|to_var_name }@::@{ group_name|to_var_name }@::_base::_@{ mod_name }@::ColOne_;
@%- endif %@
#[allow(clippy::match_single_binding)]
...
@%- else %@
pub(crate) use domain::repository::@{ db|snake|to_var_name }@::@{ group_name|to_var_name }@::_base::_@{ mod_name }@::ColKey_;
@%- endif %@
#[allow(clippy::match_single_binding)]
...
@%- else %@
pub(crate) use domain::repository::@{ db|snake|to_var_name }@::@{ group_name|to_var_name }@::_base::_@{ mod_name }@::ColMany_;
@%- endif %@
#[allow(clippy::match_single_binding)]
...
@%- else %@
pub(crate) use domain::repository::@{ db|snake|to_var_name }@::@{ group_name|to_var_name }@::_base::_@{ mod_name }@::ColJson_;
@%- endif %@
#[allow(clippy::match_single_binding)]
...
@%- else %@
pub(crate) use domain::repository::@{ db|snake|to_var_name }@::@{ group_name|to_var_name }@::_base::_@{ mod_name }@::ColJsonArray_;
@%- endif %@
#[allow(clippy::match_single_binding)]
...
@%- else %@
pub(crate) use domain::repository::@{ db|snake|to_var_name }@::@{ group_name|to_var_name }@::_base::_@{ mod_name }@::ColGeo_;
@%- endif %@
#[allow(clippy::match_single_binding)]
...
@%- else %@
pub(crate) use domain::repository::@{ db|snake|to_var_name }@::@{ group_name|to_var_name }@::_base::_@{ mod_name }@::ColGeoDistance_;
@%- endif %@
#[allow(clippy::match_single_binding)]
...
@%- else %@
pub(crate) use domain::repository::@{ db|snake|to_var_name }@::@{ group_name|to_var_name }@::_base::_@{ mod_name }@::ColRel_;
@%- endif %@
...
@%- else %@
pub(crate) use domain::repository::@{ db|snake|to_var_name }@::@{ group_name|to_var_name }@::_base::_@{ mod_name }@::Filter_;
@%- endif %@
...
@%- else %@
pub(crate) use domain::repository::@{ db|snake|to_var_name }@::@{ group_name|to_var_name }@::_base::_@{ mod_name }@::Order_;
@%- endif %@
...
impl IdTr<_@{pascal_name}@, _@{pascal_name}@Cache, _@{pascal_name}@Updater> for @{ id_name }@ {
...
@%- endif %@
// TODO @{ id_name }@FetcherとIdTr統合

pub trait @{ id_name }@Fetcher {
    fn fetch(&self, conn: &mut DbConn) -> impl std::future::Future<Output = Result<Option<_@{ pascal_name }@>>> + Send;
@%- if def.use_cache() %@
    fn fetch_from_cache(&self, conn: &DbConn) -> impl std::future::Future<Output = Result<Option<_@{ pascal_name }@Cache>>> + Send;
    @%- if def.is_soft_delete() %@
    fn fetch_from_cache_with_trashed(&self, conn: &DbConn) -> impl std::future::Future<Output = Result<Option<_@{ pascal_name }@Cache>>> + Send;
    @%- endif %@
@%- endif %@
}
@%- if def.primaries().len() == 1 && def.use_cache() %@

impl @{ id_name }@Fetcher for @{ id_name }@ {
    async fn fetch(&self, conn: &mut DbConn) -> Result<Option<_@{ pascal_name }@>> {
        _@{ pascal_name }@::find_optional(conn, self, None).await
    }
@%- if def.use_cache() %@
    ...
    #[cfg(not(feature="cache_update_only"))]
    async fn fetch_from_cache_with_trashed(&self, conn: &DbConn) -> Result<Option<_@{ pascal_name }@Cache>> {
        _@{ pascal_name }@::find_from_cache_with_trashed(conn, self).await.map(Some)
    }
    @%- endif %@
@%- endif %@
}
...
impl @{ id_name }@Fetcher for Option<@{ id_name }@> {
    async fn fetch(&self, conn: &mut DbConn) -> Result<Option<_@{ pascal_name }@>> {
        if let Some(id) = self {
            _@{ pascal_name }@::find_optional(conn, id, None).await
        } else {
            Ok(None)
        }
    }
...
impl CacheTr<Primary, _@{ pascal_name }@Cache> for _@{ pascal_name }@Cache {
    async fn invalidate_cache<T>(conn: &DbConn, id: T) -> Result<()>
    where
        T: Into<Primary>,
    {
    ...
    async fn __push_invalidate_cache_op<T>(conn: &mut DbConn, id: T) -> Result<()>
    where
        T: Into<Primary>,
    {
    ...
    async fn __push_notify_cache_op<T>(conn: &mut DbConn, id: T) -> Result<()>
    where
        T: Into<Primary>,
    {
        @%- if def.act_as_job_queue() || def.use_clear_whole_cache() %@
    ...
    @%- if def.use_cache() %@
    #[cfg(feature="cache_update_only")]
    async fn find_self(&self, conn: &DbConn) -> Result<Self> {
        unimplemented!("cache_update_only feature disables fetching from cache.")
    }
    #[cfg(not(feature="cache_update_only"))]
    async fn find_self(&self, conn: &DbConn) -> Result<Self> {
...
impl ModelTr<Primary, _@{ pascal_name }@, _@{ pascal_name }@Cache, _@{ pascal_name }@Updater, Filter_, Order_, QueryBuilder> for _@{ pascal_name }@ {
...
@%- endif %@
    ...
    async fn find<T>(conn: &mut DbConn, id: T, filter: Option<Filter_>) -> Result<_@{ pascal_name }@>
    where
        T: Into<Primary>,
    {
    ...
    async fn find_with_trashed<T>(conn: &mut DbConn, id: T, filter: Option<Filter_>) -> Result<_@{ pascal_name }@>
    where
        T: Into<Primary>,
    {
        let id: Primary = id.into();
    ...
    #[cfg(not(feature="cache_update_only"))]
    async fn find_from_cache<T>(conn: &DbConn, id: T) -> Result<_@{ pascal_name }@Cache>
    where
        T: Into<Primary>,
    {
        let id: InnerPrimary = (&id.into()).into();
...
@{- def.soft_delete_tpl("","
    ...
    #[cfg(not(feature="cache_update_only"))]
    async fn find_from_cache_with_trashed<T>(conn: &DbConn, id: T) -> Result<_@{ pascal_name }@Cache>
    where
        T: Into<Primary>,
    {
        let id: InnerPrimary = (&id.into()).into();
    ...
    fn list_to_map(list: Vec<_@{ pascal_name }@>) -> AHashMap<Primary, _@{ pascal_name }@> {
        let mut map = AHashMap::default();
        for v in list {
            map.insert((&v).into(), v);
        }
    ...
    fn cache_list_to_map(list: Vec<_@{ pascal_name }@Cache>) -> AHashMap<Primary, _@{ pascal_name }@Cache> {
        let mut map = AHashMap::default();
        for v in list {
            map.insert((&v).into(), v);
        }
    ...
    fn updater_list_to_map(list: Vec<_@{ pascal_name }@Updater>) -> AHashMap<Primary, _@{ pascal_name }@Updater> {
        let mut map = AHashMap::default();
        for v in list {
            map.insert((&v).into(), v);
        }
    ...
    async fn find_many<I, T>(conn: &mut DbConn, ids: I, filter: Option<Filter_>) -> Result<Vec<_@{ pascal_name }@>>
    where
    ...
    async fn find_many_with_trashed<I, T>(conn: &mut DbConn, ids: I, filter: Option<Filter_>) -> Result<Vec<_@{ pascal_name }@>>
    where
        I: IntoIterator<Item = T>,
        T: Into<Primary>,
    {
    ...
    #[cfg(not(feature="cache_update_only"))]
    async fn find_many_from_cache<I, T>(conn: &DbConn, ids: I) -> Result<Vec<_@{ pascal_name }@Cache>>
    where
        I: IntoIterator<Item = T>,
        T: Into<Primary>,
    {
    ...
    #[cfg(not(feature="cache_update_only"))]
    async fn find_many_from_cache_with_trashed<I, T>(conn: &DbConn, ids: I) -> Result<Vec<_@{ pascal_name }@Cache>>
    where
        I: IntoIterator<Item = T>,
        T: Into<Primary>,
    {
    ...
    async fn find_optional<T>(conn: &mut DbConn, id: T, filter: Option<Filter_>) -> Result<Option<_@{ pascal_name }@>>
    where
        T: Into<Primary>,
    {
        let id: InnerPrimary = (&id.into()).into();
    ...
    async fn exists<T>(conn: &mut DbConn, id: T, filter: Option<Filter_>) -> Result<bool>
    where
        T: Into<Primary>,
    {
        let id: InnerPrimary = (&id.into()).into();
    ...
    async fn find_optional_for_update<T>(conn: &mut DbConn, id: T, filter: Option<Filter_>) -> Result<Option<_@{ pascal_name }@Updater>>
    where
        T: Into<Primary>,
    {
        let id: InnerPrimary = (&id.into()).into();
    ...
    async fn find_optional_with_trashed<T>(conn: &mut DbConn, id: T, filter: Option<Filter_>) -> Result<Option<_@{ pascal_name }@>>
    where
        T: Into<Primary>,
    {
        let id: InnerPrimary = (&id.into()).into();
    ...
    async fn exists_with_trashed<T>(conn: &mut DbConn, id: T, filter: Option<Filter_>) -> Result<bool>
    where
        T: Into<Primary>,
    {
        let id: InnerPrimary = (&id.into()).into();
    ...
    async fn find_optional_from_cache<T>(conn: &DbConn, id: T) -> Result<Option<_@{ pascal_name }@Cache>>
    where
        T: Into<Primary>,
    {
        let id: InnerPrimary = (&id.into()).into();
    ...
    #[cfg(not(feature="cache_update_only"))]
    async fn find_optional_from_cache_with_trashed<T>(conn: &DbConn, id: T) -> Result<Option<_@{ pascal_name }@Cache>>
    where
        T: Into<Primary>,
    {
        let id: InnerPrimary = (&id.into()).into();
    ...
    async fn find_for_update<T>(conn: &mut DbConn, id: T, filter: Option<Filter_>) -> Result<_@{ pascal_name }@Updater>
    where
        T: Into<Primary>,
    {
        let id: InnerPrimary = (&id.into()).into();
    ...
    async fn find_for_update_with_trashed<T>(conn: &mut DbConn, id: T, filter: Option<Filter_>) -> Result<_@{ pascal_name }@Updater>
    where
        T: Into<Primary>,
    {
        let id: InnerPrimary = (&id.into()).into();
...
@%- if !def.disable_update() %@
    ...
    async fn find_many_for_update<I, T>(conn: &mut DbConn, ids: I, filter: Option<Filter_>) -> Result<Vec<_@{ pascal_name }@Updater>>
    where
        I: IntoIterator<Item = T>,
        T: Into<Primary>,
    {
    ...
    async fn find_many_for_update_with_trashed<I, T>(conn: &mut DbConn, ids: I, filter: Option<Filter_>) -> Result<Vec<_@{ pascal_name }@Updater>>
    where
        I: IntoIterator<Item = T>,
        T: Into<Primary>,
    {
...
@%- for (index_name, index) in def.unique_index() %@
@#
    async fn find_by_@{ index_name }@<@{ index.fields(index_name, def)|fmt_index_col("T{index}", ", ") }@>(conn: &mut DbConn, @{ index.fields(index_name, def)|fmt_index_col("_{name}: T{index}", ", ") }@) -> Result<_@{ pascal_name }@>
    where
...
@#
    #[cfg(feature="cache_update_only")]
    async fn find_by_@{ index_name }@_from_cache<@{ index.fields(index_name, def)|fmt_index_col("T{index}", ", ") }@>(conn: &DbConn, @{ index.fields(index_name, def)|fmt_index_col("_{name}: T{index}", ", ") }@) -> Result<_@{ pascal_name }@Cache>
    where
    @{- index.fields(index_name, def)|fmt_index_col("
        T{index}: Into<{filter_type}>,", "") }@
    {
...
@%- if !def.act_as_job_queue() && !def.use_clear_whole_cache() %@
    ...
    #[cfg(not(feature="cache_update_only"))]
    async fn insert_dummy_cache(conn: &DbConn, obj: _@{ pascal_name }@Updater) -> Result<()> {
...
@%- endif %@
    ...
    async fn save(conn: &mut DbConn, mut obj: _@{ pascal_name }@Updater) -> Result<Option<_@{ pascal_name }@>> {
        obj.__validate()?;
        obj.__set_default_value(conn)@% if config.use_sequence %@.await?@% endif %@;
        __save(conn, obj, 0).await
    }

    async fn replace(conn: &mut DbConn, mut obj: _@{ pascal_name }@Updater) -> Result<Option<_@{ pascal_name }@>> {
        obj.__validate()?;
    ...
    async fn update_many<I, T>(conn: &mut DbConn, ids: I, mut updater: _@{ pascal_name }@Updater) -> Result<u64>
    where
        I: IntoIterator<Item = T>,
        T: Into<Primary>,
    {
    ...
    async fn insert_ignore(conn: &mut DbConn, mut obj: _@{ pascal_name }@Updater) -> Result<Option<_@{ pascal_name }@Updater>> {
        obj.__validate()?;
        ensure!(obj.is_new(), "The obj is not new.");
        obj.__set_default_value(conn)@% if config.use_sequence %@.await?@% endif %@;
...
@{- def.relations_many_cache(false)|fmt_rel_join("\n                _{rel_name}: None,", "") }@
    ...
    async fn insert_delayed(conn: &mut DbConn, mut obj: _@{ pascal_name }@Updater) -> Result<()> {
        ensure!(obj.is_new(), "The obj is not new.");
        obj.__validate()?;
        obj.__set_default_value(conn)@% if config.use_sequence %@.await?@% endif %@;
    ...
    async fn save_delayed(conn: &mut DbConn, mut obj: _@{ pascal_name }@Updater) -> Result<()> {
        obj.__validate()?;
        obj.__set_default_value(conn)@% if config.use_sequence %@.await?@% endif %@;
...
@{- def.relations_belonging(false)|fmt_rel_join("\n        obj.{rel_name} = None;", "") }@
    ...
    async fn update_delayed(conn: &mut DbConn, mut obj: _@{ pascal_name }@Updater) -> Result<()> {
        if obj.is_new() {
...
@{- def.relations_belonging(false)|fmt_rel_join("\n        obj.{rel_name} = None;", "") }@
    ...
    async fn upsert_delayed(conn: &mut DbConn, mut obj: _@{ pascal_name }@Updater) -> Result<()> {
        obj.__validate()?;
        @%- if def.soft_delete().is_none() %@
        if obj.will_be_deleted() {
            panic!("DELETE is not supported.");
...
@{- def.relations_belonging(false)|fmt_rel_join("\n        obj.{rel_name} = None;", "") }@
    ...
    async fn bulk_insert(conn: &mut DbConn, mut list: Vec<_@{ pascal_name }@Updater>, ignore: bool) -> Result<()> {
        _bulk_insert(conn, list, ignore, false).await
    }
@%- if !def.disable_update() %@

    async fn bulk_replace(conn: &mut DbConn, mut list: Vec<_@{ pascal_name }@Updater>) -> Result<()> {
        _bulk_insert(conn, list, false, true).await
    }

    async fn bulk_overwrite(conn: &mut DbConn, mut list: Vec<_@{ pascal_name }@Updater>) -> Result<()> {
...
@%- endif %@
    ...
    async fn bulk_upsert(conn: &mut DbConn, mut list: Vec<_@{ pascal_name }@Updater>, mut updater: _@{ pascal_name }@Updater) -> Result<()> {
        if list.is_empty() {
            return Ok(());
        }
...
@%- if !def.disable_update() %@
    ...
    async fn delete_by_ids<I, T>(conn: &mut DbConn, ids: I) -> Result<u64>
    where
...
@%- endif %@
    ...
    #[allow(clippy::needless_borrow)]
    async fn force_delete_by_ids<I, T>(conn: &mut DbConn, ids: I) -> Result<u64>
    where
        I: IntoIterator<Item = T>,
        T: Into<Primary>,
    {
...
@%- if !def.disable_update() %@
    ...
    #[allow(unused_mut)]
    async fn delete(conn: &mut DbConn, mut obj: _@{ pascal_name }@Updater) -> Result<()> {
        @%- if def.updated_at_conf().is_some() %@
        if obj._op.@{ ConfigDef::updated_at()|to_var_name }@ == Op::None {
            obj.mut_@{ ConfigDef::updated_at() }@().set(@{(def.updated_at_conf().unwrap() == Timestampable::RealTime)|if_then_else_ref("SystemTime::now()","conn.time()")}@.into());
        }
...
@%- if !def.disable_update() && def.soft_delete().is_some() %@
    ...
    #[allow(unused_mut)]
    async fn restore(conn: &mut DbConn, mut obj: _@{ pascal_name }@Updater) -> Result<_@{ pascal_name }@> {
...
@%- if !def.disable_update() %@
    ...
    #[allow(clippy::needless_borrow)]
    async fn force_delete(conn: &mut DbConn, obj: _@{ pascal_name }@Updater) -> Result<()> {
...
@%- if def.use_on_delete_fn %@
        ...
        _before_delete(conn, &[notify_obj.clone()]).await?;
@%- endif %@
@%- for on_delete_str in def.on_delete_list %@
...
@%- if def.use_on_delete_fn %@
        conn.push_callback(Box::new(|| {
            async {
                _after_delete(&[notify_obj]).await;
            }.boxed()
        })).await;
@%- endif %@
@%- if !config.force_disable_cache %@
...
@%- endif %@
    ...
    async fn force_delete_relations(conn: &mut DbConn, obj: _@{ pascal_name }@Updater) -> Result<()> {
        let id: InnerPrimary = (&obj).into();
@%- for on_delete_str in def.on_delete_list %@
...
@%- endfor %@
    ...
    async fn force_delete_all(conn: &mut DbConn) -> Result<()> {
    ...
    async fn truncate(conn: &mut DbConn) -> Result<()> {
...
async fn __update_many(conn: &mut DbConn, ids: Vec<InnerPrimary>, mut obj: _Updater_) -> Result<u64> {
    ...
    if !conn.clear_whole_cache && (USE_CACHE || USE_ALL_ROWS_CACHE || USE_UPDATE_NOTICE) {
        ...
        if has_add {
            for ids in ids.chunks(IN_CONDITION_LIMIT) {
                _@{ pascal_name }@::find_many_for_update(conn, ids.iter(), None).await?.into_iter()
                    .for_each(|v| {
...
@{- def.soft_delete_tpl2("
    _--1--::force_delete(conn, obj).await?;
    Ok(None)","
    obj.mut_deleted_at().set(Some({val}.into()));
