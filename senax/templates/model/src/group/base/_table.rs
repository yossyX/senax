...
use crate::misc::{BindArrayTr, BindTr, ColRelTr, ColTr, FilterTr, ToJsonRawValue as _, IntoJson as _, OrderTr};
            ...
            loop {
                ...
                for inner_map in buf.iter() {
                    for (_op, updater) in inner_map.iter() {
                        let result = _@{ pascal_name }@::__save(&mut conn, updater.clone(), 0).await;
                        if crate::connection::_is_retryable_error(result, TABLE_NAME) {
...
pub@{ visibility }@ fn write_belonging_rel(buf: &mut String, filter: &Option<Box<Filter_>>, cols: String, idx: usize, without_key: bool, shard_id: ShardId, is_outer: bool) {
    let db = if is_outer {
...
pub@{ visibility }@ fn write_having_rel(buf: &mut String, filter: &Option<Box<Filter_>>, cols1: &str, cols2: String, cols3: &str, idx: usize, without_key: bool, shard_id: ShardId, is_outer: bool) {
    let db = if is_outer {
        DbConn::real_db_name(shard_id)
    } else {
        ""
...
pub@{ visibility }@ fn bind_for_rel_query(filter: Option<Box<Filter_>>, query: Query<'_, DbType, DbArguments>) -> Query<'_, DbType, DbArguments> {
    if let Some(filter) = filter {
...
impl std::fmt::Display for Filter_ {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Filter_::Match(cols, _) => write!(f, "Match:<{}>", cols.iter().map(|v| v.name()).collect::<Vec<_>>().join(",")),
            Filter_::MatchBoolean(cols, _) => write!(f, "MatchBoolean:<{}>", cols.iter().map(|v| v.name()).collect::<Vec<_>>().join(",")),
            Filter_::MatchExpansion(cols, _) => write!(f, "MatchExpansion:<{}>", cols.iter().map(|v| v.name()).collect::<Vec<_>>().join(",")),
            Filter_::IsNull(col) => write!(f, "IsNull:{}", col.name()),
            Filter_::IsNotNull(col) => write!(f, "IsNotNull:{}", col.name()),
            Filter_::Eq(col) => write!(f, "Eq:{}", col.name()),
            Filter_::EqKey(col) => write!(f, "EqKey:{}", col.name()),
            Filter_::NotEq(col) => write!(f, "NotEq:{}", col.name()),
            Filter_::Gt(col) => write!(f, "Gt:{}", col.name()),
            Filter_::Gte(col) => write!(f, "Gte:{}", col.name()),
            Filter_::Lt(col) => write!(f, "Lt:{}", col.name()),
            Filter_::Lte(col) => write!(f, "Lte:{}", col.name()),
            Filter_::Like(col) => write!(f, "Like:{}", col.name()),
            Filter_::AllBits(col) => write!(f, "AllBits:{}", col.name()),
            Filter_::AnyBits(col) => write!(f, "AnyBits:{}", col.name()),
            Filter_::In(col) => write!(f, "In:{}", col.name()),
            Filter_::NotIn(col) => write!(f, "NotIn:{}", col.name()),
            Filter_::MemberOf(col, _) => write!(f, "MemberOf:{}", col.name()),
            Filter_::Contains(col, _) => write!(f, "Contains:{}", col.name()),
            Filter_::Overlaps(col, _) => write!(f, "Overlaps:{}", col.name()),
            Filter_::JsonIn(col, _) => write!(f, "JsonIn:{}", col.name()),
            Filter_::JsonContainsPath(col, _) => write!(f, "JsonContainsPath:{}", col.name()),
            Filter_::JsonEq(col, _) => write!(f, "JsonEq:{}", col.name()),
            Filter_::JsonLt(col, _) => write!(f, "JsonLt:{}", col.name()),
            Filter_::JsonLte(col, _) => write!(f, "JsonLte:{}", col.name()),
            Filter_::JsonGt(col, _) => write!(f, "JsonGt:{}", col.name()),
            Filter_::JsonGte(col, _) => write!(f, "JsonGte:{}", col.name()),
            Filter_::Within(col) => write!(f, "Within:{}", col.name()),
            Filter_::Intersects(col) => write!(f, "Intersects:{}", col.name()),
            Filter_::Crosses(col) => write!(f, "Crosses:{}", col.name()),
            Filter_::DWithin(col) => write!(f, "DWithin:{}", col.name()),
