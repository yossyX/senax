...
use crate::misc::{Updater, Size, TrashMode};
...
use crate::repository::@{ mod_name[0]|to_var_name }@::_base::_@{ mod_name[1] }@ as rel_@{ mod_name[0] }@_@{ mod_name[1] }@;
@%- if !config.exclude_from_domain %@
...
@%- endfor %@
@%- for (name, rel_def) in def.belongs_to_outer_db() %@
use db_@{ rel_def.db() }@::repository::@{ rel_def.get_base_group_mod_var() }@ as rel_@{ rel_def.get_group_mod_name() }@;
@%- endfor %@
...
#[derive(Deserialize, Serialize, Clone, Debug)]
pub struct PrimaryWrapper(InnerPrimary, ShardId, MSec);
@%- if !config.force_disable_cache %@
...
impl From<domain::repository::@{ db|snake|to_var_name }@::@{ group_name|to_var_name }@::@{ mod_name|to_var_name }@::@{ pascal_name }@Factory> for _@{ pascal_name }@Updater {
    fn from(v: domain::repository::@{ db|snake|to_var_name }@::@{ group_name|to_var_name }@::@{ mod_name|to_var_name }@::@{ pascal_name }@Factory) -> Self {
        Self {
            _data: Data {
...
@%- for parent in def.downcast_aggregation() %@
    ...
    pub fn downcast_from(base: &crate::models::@{ parent.group_name|to_var_name }@::_base::_@{ parent.name }@::_@{ parent.name|pascal }@) -> Option<_@{ pascal_name }@> {
...
@%- for parent in def.downcast_simple() %@
    ...
    pub fn downcast_from(base: &crate::models::@{ parent.group_name|to_var_name }@::_base::_@{ parent.name }@::_@{ parent.name|pascal }@) -> _@{ pascal_name }@ {
        let clone = base._inner.clone();
        Self {
            _inner: Data {
                @{- def.all_fields()|fmt_join("
