...
pub fn write_base_group_files(
    ...
    #[derive(Template)]
    #[template(path = "db/base_repositories/src/misc.rs", escape = "none")]
    ...
    for (group_name, (f, defs, _)) in groups {
        ...
        for (model_name, (_, def)) in defs {
            ...
            if !def.abstract_mode {
                ...
                #[derive(Template)]
                #[template(path = "db/base_repositories/src/group/base/_table.rs", escape = "none")]
...
pub fn write_group_files(
    ...
    let file_path = base_dir.join("Cargo.toml");
    remove_files.remove(file_path.as_os_str());
    let act_as_session = groups.values().any(|g| g.1.values().any(|m| m.1.act_as_session()));
    if force || !file_path.exists() {
        #[derive(Template)]
        ...
        let content = Template { db, group, act_as_session }.render()?;
        fs_write(file_path, &*content)?;
    }
