...
pub fn reflect_migration_changes(db: &str) -> anyhow::Result<()> {
    ...
    for (group_name, group_def) in config.groups {
        ...
        for (name, model) in models.iter_mut() {
            ...
            for (field_name, field) in model.fields.clone().into_iter() {
                let mut field = field.exact();
                field._before_rename_name = Some(field.get_col_name(&field_name).to_string());
                if field
                    ._before_rename_name
                    .as_deref()
                    .unwrap()
                ...
                model
                    .fields
                    .insert(field_name, schema::FieldDefOrSubsetType::Exact(field));
            }
        }
...
impl AtomicLoad<usize> for AtomicUsize {
